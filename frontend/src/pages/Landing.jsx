...
export default function Landing() {
  ...
  const submit = async (e) => {
    ...
    try { const user = mode === "login" ? await login(form.email, form.password) : await register(form.email, form.password, form.name); toast.success(`Welcome ${user.name?.split(" ")[0] || ""}!`); routeUser(user); }
