...
def image_has_ai_provenance(data: bytes) -> bool:
    """Reject files that explicitly identify themselves as AI-generated.
    This is deliberately conservative: it rejects explicit AI provenance strings
    but does not claim to detect every AI image. Face/liveness matching remains
    ...
    markers = (
        b"stable diffusion", b"midjourney", b"dall-e", b"dall·e", b"openai",
        b"adobe firefly", b"comfyui", b"leonardo.ai", b"ideogram", b"generative ai",
        b"ai generated", b"ai-generated", b"synthetic media"
    )
...
class OnboardingData(BaseModel):
    age: int; gender: str; city: str; locality: str
    home_lat: Optional[float] = None; home_lng: Optional[float] = None; radius_km: int = 5
    office_locality: Optional[str] = ""; office_lat: Optional[float] = None; office_lng: Optional[float] = None
    housing_status: str; budget_min: int; budget_max: int; move_in_date: str; flatmate_gender_pref: str
    work_profile: str; company_or_college: Optional[str] = ""; work_schedule: str = "hybrid"
    food_pref: str; cooks_at_home: str; cleanliness: int; sleep_schedule: str; social_level: str
    drinking: str; smoking: str; pets_ok: bool; guests_freq: str; male_guests_ok: bool = True
    family_visits_ok: bool = True; hosts_parties: str = "sometimes"; music_ok: bool
    languages: List[str] = []; interests: List[str] = []; flat_preferences: List[str] = []
class NonNegotiablesIn(BaseModel):
    non_negotiables: List[str] = []
class ProfileUpdate(BaseModel):
    bio: Optional[str] = None; prompts: Optional[List[Dict[str, str]]] = None
    photos: Optional[List[str]] = None; flat_photos: Optional[List[str]] = None
    main_photo_verified: Optional[bool] = None; face_descriptor_main: Optional[List[float]] = None
class EditProfileIn(BaseModel):
    age: Optional[int] = None; gender: Optional[str] = None; city: Optional[str] = None; locality: Optional[str] = None
    home_lat: Optional[float] = None; home_lng: Optional[float] = None; radius_km: Optional[int] = None
    office_locality: Optional[str] = None; office_lat: Optional[float] = None; office_lng: Optional[float] = None
    housing_status: Optional[str] = None; budget_min: Optional[int] = None; budget_max: Optional[int] = None
    move_in_date: Optional[str] = None; flatmate_gender_pref: Optional[str] = None; work_profile: Optional[str] = None
    company_or_college: Optional[str] = None; work_schedule: Optional[str] = None; food_pref: Optional[str] = None
    cooks_at_home: Optional[str] = None; cleanliness: Optional[int] = None; sleep_schedule: Optional[str] = None
    social_level: Optional[str] = None; drinking: Optional[str] = None; smoking: Optional[str] = None
    pets_ok: Optional[bool] = None; guests_freq: Optional[str] = None; male_guests_ok: Optional[bool] = None
    family_visits_ok: Optional[bool] = None; hosts_parties: Optional[str] = None; music_ok: Optional[bool] = None
    languages: Optional[List[str]] = None; interests: Optional[List[str]] = None; flat_preferences: Optional[List[str]] = None
    bio: Optional[str] = None; prompts: Optional[List[Dict[str, str]]] = None
...
async def register(data: UserRegister):
    ...
    doc={"user_id":user_id,"email":data.email,"name":data.name,"password_hash":hash_password(data.password),"onboarding_done":False,"liveness_verified":False,"profile_complete":False,"main_photo_verified":False,"created_at":datetime.now(timezone.utc).isoformat()}
    await db.users.insert_one(doc); return {"token":create_jwt(user_id),"user":user_public(doc)}

@api_router.post("/auth/login")
async def login(data: UserLogin):
    user=await db.users.find_one({"email":data.email},{"_id":0})
    if not user or not user.get("password_hash") or not verify_password(data.password,user["password_hash"]): raise HTTPException(status_code=401,detail="Invalid credentials")
...
async def process_session(request: Request, response: Response):
    ...
    data=resp.json(); email=data["email"]; user=await db.users.find_one({"email":email},{"_id":0})
    if not user:
        user={"user_id":f"user_{uuid.uuid4().hex[:12]}","email":email,"name":data.get("name") or email.split("@")[0],"picture":data.get("picture"),"onboarding_done":False,"liveness_verified":False,"profile_complete":False,"main_photo_verified":False,"created_at":datetime.now(timezone.utc).isoformat()}
        await db.users.insert_one(user)
    session_token=data["session_token"]; expires_at=datetime.now(timezone.utc)+timedelta(days=7)
...
async def delete_account(user: dict = Depends(get_current_user)):
    uid=user["user_id"]
    matches=await db.matches.find({"user_ids":uid},{"_id":0,"match_id":1}).to_list(None); match_ids=[m["match_id"] for m in matches if m.get("match_id")]
    if match_ids: await db.messages.delete_many({"match_id":{"$in":match_ids}})
    await db.messages.delete_many({"sender_id":uid}); await db.matches.delete_many({"user_ids":uid}); await db.swipes.delete_many({"$or":[{"user_id":uid},{"target_user_id":uid}]}); await db.user_sessions.delete_many({"user_id":uid}); await db.users.delete_one({"user_id":uid})
...
async def verify_liveness(body:Dict[str,Any],user:dict=Depends(get_current_user)):
    ...
    await db.users.update_one({"user_id":user["user_id"]},{"$set":{"liveness_verified":True,"selfie_path":selfie_path,"face_descriptor_selfie":face_desc}})
...
async def update_profile(data:ProfileUpdate,user:dict=Depends(get_current_user)):
    upd={k:v for k,v in data.model_dump().items() if v is not None}; photos=upd.get("photos",user.get("photos",[]) or []); flat_photos=upd.get("flat_photos",user.get("flat_photos",[]) or []); housing=user.get("housing_status")
    prev_main=(user.get("photos") or [None])[0]; new_main=(photos or [None])[0]; face_desc_main=upd.pop("face_descriptor_main",None)
    if new_main and new_main!=prev_main:
        ...
        if dist>0.6:
            upd["main_photo_verified"]=False; raise HTTPException(status_code=400,detail=f"Main photo does not match your live selfie (distance {dist:.2f}). Please upload a photo of yourself.")
    ...
    upd["profile_complete"]=bool(photos) and bool(upd.get("prompts") or user.get("prompts")) and (housing!="have_house" or bool(flat_photos))
    await db.users.update_one({"user_id":user["user_id"]},{"$set":upd}); updated=await db.users.find_one({"user_id":user["user_id"]},{"_id":0}); return user_public(updated)
...
async def edit_profile(data:EditProfileIn,user:dict=Depends(get_current_user)):
    ...
    if new_housing=="have_house" and len(user.get("flat_photos") or [])<1: raise HTTPException(status_code=400,detail="Please add at least 1 flat photo from 'Manage photos' before switching to 'I have a house'")
    await db.users.update_one({"user_id":user["user_id"]},{"$set":upd}); updated=await db.users.find_one({"user_id":user["user_id"]},{"_id":0}); return user_public(updated)
...
async def upload_photo(file:UploadFile=File(...),user:dict=Depends(get_current_user)):
    ...
    if image_has_ai_provenance(data): raise HTTPException(status_code=400,detail="This image contains AI-generation metadata. Please upload a real photo taken by you.")
    path=f"{APP_NAME}/uploads/{user['user_id']}/{uuid.uuid4().hex}.{ext}"; content_type=file.content_type or f"image/{'jpeg' if ext in ('jpg','jpeg') else ext}"; result=put_object(path,data,content_type)
    await db.files.insert_one({"id":str(uuid.uuid4()),"user_id":user["user_id"],"storage_path":result["path"],"content_type":content_type,"size":result["size"],"is_deleted":False,"created_at":datetime.now(timezone.utc).isoformat()})
...
async def serve_file(path:str):
    ...
    if not record:
        ...
        if not selfie_user: raise HTTPException(status_code=404,detail="Not found")
...
def haversine_km(lat1,lng1,lat2,lng2):
    ...
    dlat=radians(lat2-lat1); dlng=radians(lng2-lng1); a=sin(dlat/2)**2+cos(radians(lat1))*cos(radians(lat2))*sin(dlng/2)**2
...
def compute_match(a:dict,b:dict)->Dict[str,Any]:
    ...
    dist=haversine_km(a.get("home_lat"),a.get("home_lng"),b.get("home_lat"),b.get("home_lng")); same_locality=a.get("locality") and a.get("locality").strip().lower()==(b.get("locality") or "").strip().lower()
    ...
    ah,bh=a.get("housing_status"),b.get("housing_status"); good_pairs={("have_house","need_house_from_someone"),("need_house_from_someone","have_house"),("need_house_together","need_house_together")}; add((ah,bh) in good_pairs,"housing_complementary",15)
    a_min,a_max=a.get("budget_min",0),a.get("budget_max",999999); b_min,b_max=b.get("budget_min",0),b.get("budget_max",999999); add(a_max>=b_min and b_max>=a_min,"budget_overlap",10)
    add(a.get("food_pref")==b.get("food_pref") and a.get("food_pref"),"food_same",8); ca,cb=a.get("cleanliness"),b.get("cleanliness"); add(isinstance(ca,int) and isinstance(cb,int) and abs(ca-cb)<=1,"clean_same",8); add(a.get("sleep_schedule")==b.get("sleep_schedule") and a.get("sleep_schedule"),"sleep_same",6); add(a.get("social_level")==b.get("social_level") and a.get("social_level"),"social_same",5); add(a.get("smoking")==b.get("smoking") and a.get("smoking"),"smoking_same",4); add(a.get("drinking")==b.get("drinking") and a.get("drinking"),"drinking_same",4); add(a.get("pets_ok")==b.get("pets_ok"),"pets_same",3); add(a.get("guests_freq")==b.get("guests_freq") and a.get("guests_freq"),"guests_align",3); add(a.get("work_schedule")==b.get("work_schedule") and a.get("work_schedule"),"work_schedule_same",4)
    overlap=len(set(a.get("interests") or [])&set(b.get("interests") or [])); add(overlap>=2,"interests_shared",min(6,overlap*2)); add(bool(set(a.get("languages") or [])&set(b.get("languages") or [])),"languages_shared",3)
    return {"score":min(100,score),"highlights":highlights,"distance_km":dist}

def gender_ok(x,y):
    pref=x.get("flatmate_gender_pref","any"); return pref=="any" or pref==y.get("gender")
def city_key(value):
    v=(value or "").strip().lower()
    return "bangalore" if v in ("bangalore","bengaluru") else v

def passes_hard_filters(a:dict,b:dict,filters:Dict[str,Any])->bool:
    non_neg=set(a.get("non_negotiables") or [])
    if "food_pref" in non_neg and a.get("food_pref")!=b.get("food_pref"): return False
    if "smoking" in non_neg and a.get("smoking")!=b.get("smoking"): return False
    if "drinking" in non_neg and a.get("drinking")!=b.get("drinking"): return False
    if "pets_ok" in non_neg and a.get("pets_ok")!=b.get("pets_ok"): return False
    if "male_guests_ok" in non_neg and a.get("male_guests_ok")!=b.get("male_guests_ok"): return False
    if "family_visits_ok" in non_neg and a.get("family_visits_ok")!=b.get("family_visits_ok"): return False
...
async def discover(user:dict=Depends(get_current_user),radius_km:Optional[int]=None,food:Optional[str]=None,smoking:Optional[str]=None,drinking:Optional[str]=None,housing:Optional[str]=None,budget_min:Optional[int]=None,budget_max:Optional[int]=None):
    if not user.get("onboarding_done"): raise HTTPException(status_code=400,detail="Complete onboarding first")
    swiped=await db.swipes.find({"user_id":user["user_id"]},{"_id":0,"target_user_id":1}).to_list(5000); exclude_ids={s["target_user_id"] for s in swiped}; exclude_ids.add(user["user_id"])
    # Treat Bangalore and Bengaluru as the same city so geocoder naming does not empty discovery.
    candidates=await db.users.find({"user_id":{"$nin":list(exclude_ids)},"onboarding_done":True,"profile_complete":True},{"_id":0,"password_hash":0,"face_descriptor_selfie":0}).to_list(500)
    filters={"food":[x for x in (food or "").split(",") if x] or None,"smoking":[x for x in (smoking or "").split(",") if x] or None,"drinking":[x for x in (drinking or "").split(",") if x] or None,"housing":[x for x in (housing or "").split(",") if x] or None,"budget_min":budget_min,"budget_max":budget_max}; r_km=radius_km if radius_km is not None else user.get("radius_km",5)
    scored=[]
    for c in candidates:
        if city_key(user.get("city"))!=city_key(c.get("city")): continue
        if not gender_ok(user,c) or not gender_ok(c,user): continue
        if not passes_hard_filters(user,c,filters): continue
        m=compute_match(user,c); scored.append({"user":c,"score":m["score"],"highlights":m["highlights"],"distance_km":m["distance_km"]})
    same_locality=(user.get("locality") or "").strip().lower(); same_loc_bucket=[]; within_radius=[]; outside_radius=[]
    for s in scored:
        loc=(s["user"].get("locality") or "").strip().lower(); d=s["distance_km"]
        if same_locality and loc==same_locality: same_loc_bucket.append(s)
        elif d is not None and d<=r_km: within_radius.append(s)
        else: outside_radius.append(s)
    same_loc_bucket.sort(key=lambda x:-x["score"]); within_radius.sort(key=lambda x:(x["distance_km"] if x["distance_km"] is not None else 9999,-x["score"])); outside_radius.sort(key=lambda x:(x["distance_km"] if x["distance_km"] is not None else 9999,-x["score"]))
    primary=same_loc_bucket+within_radius; nearby=outside_radius[:30]; fallback_message=None
    if not primary and nearby:
        nearest=nearby[0]; d=nearest.get("distance_km"); loc=nearest["user"].get("locality") or nearest["user"].get("city"); fallback_message=f"No one in {user.get('locality') or 'your area'} yet — showing folks from {loc}"+(f" ({d:.1f}km away)" if d else "")
    return {"primary":primary[:50],"nearby":nearby if primary else nearby[:50],"fallback_message":fallback_message}
...
async def swipe(data:SwipeIn,user:dict=Depends(get_current_user)):
    ...
    other=await db.users.find_one({"user_id":data.target_user_id},{"_id":0,"password_hash":0}); m=compute_match(user,other or {})
    ...
    match_id=f"match_{uuid.uuid4().hex[:12]}"; doc={"match_id":match_id,"user_ids":[user["user_id"],data.target_user_id],"score":m["score"],"highlights":m["highlights"],"created_at":datetime.now(timezone.utc).isoformat()}; await db.matches.insert_one(doc); doc.pop("_id",None)
...
async def list_matches(user:dict=Depends(get_current_user)):
    ...
    for m in ms:
        other_id=[u for u in m["user_ids"] if u!=user["user_id"]][0]; other=await db.users.find_one({"user_id":other_id},{"_id":0,"password_hash":0,"face_descriptor_selfie":0}); last_msg=await db.messages.find_one({"match_id":m["match_id"]},{"_id":0},sort=[("created_at",-1)]); out.append({"match":m,"other":user_public(other or {}),"last_message":last_msg})
    return out
